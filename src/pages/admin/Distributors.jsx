...
import { getDistributors, createDistributor, updateDistributor, deleteDistributor } from '../../api/index';
import { PageHeader, Modal, Spinner, SearchInput, ConfirmDialog, FormField, EmptyState } from '../../components/UI';
import { Truck, Plus, Edit2, Trash2, UserCheck, UserX, Building2 } from 'lucide-react';
import toast from 'react-hot-toast';
import { useForm } from 'react-hook-form';

// Extracted common input class for cleaner form code
...
export default function AdminDistributors() {
  ...
  const MOCK = Array.from({ length: 5 }, (_, i) => ({
    distributor_id: i + 1, 
    distributor_name: `Distributor ${i + 1} Pvt Ltd`,
    primary_contact_name: `Contact Person ${i + 1}`, 
    contact_number: `9876${543210 + i}`,
    email: `dist${i + 1}@example.com`, 
    district: ['Mumbai', 'Delhi', 'Bangalore'][i % 3],
    state: ['Maharashtra', 'Delhi', 'Karnataka'][i % 3],
    gst_number: `27AABCU9603R1ZX`, 
    status: i === 3 ? 0 : 1,
    dealer_count: Math.floor(Math.random() * 10) + 1,
  ...
  const load = async () => {
    getDistributors()
      .then(r => { setDistributors(r.data); setFiltered(r.data); })
      .catch(() => { setDistributors(MOCK); setFiltered(MOCK); })
  ...
  useEffect(() => {
    ...
    setFiltered(distributors.filter(d => 
  ...
  const onSubmit = async (data) => {
    ...
    try {
      if (editData) { 
        await updateDistributor({ ...data, distributor_id: editData.distributor_id }); 
  ...
  const onDelete = async () => {
    try { 
      await deleteDistributor(deleteId); 
  ...
  return (
    <div className="animate-fade-up font-sans">
      ...
      <PageHeader 
        title="Distributors" 
        subtitle={`${distributors.length} distributors registered on platform`}
        action={
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 mb-6 mt-6">
        ...
        <div className="flex gap-3 sm:ml-auto">
          <div className="px-3 py-1.5 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-semibold flex items-center gap-1.5">
            ...
            {distributors.filter(d => d.status).length} Active
          </div>
          <div className="px-3 py-1.5 rounded-lg bg-zinc-800/50 border border-zinc-700/50 text-zinc-400 text-xs font-semibold flex items-center gap-1.5">
            ...
            {distributors.filter(d => !d.status).length} Inactive
          </div>
        </div>
      </div>
      ...
      <div className="bg-zinc-900/80 backdrop-blur-xl border border-zinc-800 rounded-3xl shadow-xl overflow-hidden">
        {filtered.length === 0 ? (
          <EmptyState 
            ...
            description={search ? "Try adjusting your search query." : "Add your first distributor to get started."} 
            action={!search && <button onClick={openCreate} className="mt-4 bg-indigo-600 hover:bg-indigo-500 text-white font-medium rounded-xl px-6 py-2.5 transition-all">Add Distributor</button>} 
          />
        ) : (
            ...
            <table className="w-full text-left border-collapse whitespace-nowrap">
              <thead>
                <tr className="bg-zinc-950/50 border-b border-zinc-800/80 text-zinc-400 text-xs uppercase tracking-wider font-semibold">
                  ...
                  <th className="px-6 py-4">Dealers</th>
              ...
              <tbody className="divide-y divide-zinc-800/50">
                {filtered.map((d, i) => (
                  <tr key={d.distributor_id} className="hover:bg-zinc-800/20 transition-colors group">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 bg-indigo-500/10 border border-indigo-500/20 rounded-xl flex items-center justify-center text-indigo-400 font-bold text-sm shadow-inner">
                          {d.distributor_name[0].toUpperCase()}
                        </div>
                        <div>
                          <p className="font-semibold text-zinc-100">{d.distributor_name}</p>
                    ...
                    <td className="px-6 py-4">
                      <p className="text-sm text-zinc-300">{d.district}</p>
                      <p className="text-xs text-zinc-500 mt-0.5">{d.state}</p>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-indigo-500/10 w-fit border border-indigo-500/20">
                        <Building2 size={14} className="text-indigo-400" />
                        <span className="font-mono text-indigo-300 font-semibold text-xs">{d.dealer_count || 0}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      {d.status ? (
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        <button onClick={() => setDeleteId(d.distributor_id)} className="p-2 text-zinc-400 hover:text-red-400 hover:bg-red-500/10 rounded-xl transition-all" title="Delete">
                          <Trash2 size={16}/>
                        </button>
                      </div>
                    </td>
      ...
      {/* Modal - Create/Edit */}
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editData ? 'Edit Distributor' : 'Add New Distributor'} size="lg">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5 mt-4">
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div className="md:col-span-2">
              <FormField label="Company Name" error={errors.distributor_name?.message}>
                <input className={inputClass} placeholder="ABC Distributors Pvt Ltd" {...register('distributor_name', { required: 'Company name is required' })} />
              </FormField>
            </div>
            
            <FormField label="Contact Person" error={errors.primary_contact_name?.message}>
              <input className={inputClass} placeholder="Rajesh Kumar" {...register('primary_contact_name', { required: 'Contact name is required' })} />
            </FormField>
            ...
            <FormField label="Email Address" error={errors.email?.message}>
              <input className={inputClass} type="email" placeholder="company@email.com" {...register('email', { required: 'Email is required' })} />
            </FormField>
            
            {!editData && (
              <FormField label="Account Password" error={errors.password?.message}>
                <input className={inputClass} type="password" placeholder="Set login password" {...register('password', { required: 'Password is required', minLength: { value: 6, message: 'Minimum 6 characters' } })} />
              </FormField>
            )}
            
            <FormField label="GST Number">
              <input className={inputClass} placeholder="27AABCU9603R1ZX" {...register('gst_number')} />
            </FormField>
            
            <FormField label="PAN Number">
              <input className={inputClass} placeholder="AABCU9603R" {...register('pan_number')} />
            </FormField>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            <FormField label="Bank Name">
              <input className={inputClass} placeholder="HDFC Bank" {...register('bank_name')} />
            </FormField>
            <FormField label="Account Number">
              <input className={inputClass} placeholder="12345678901" {...register('account_number')} />
            </FormField>
            <FormField label="IFSC Code">
              <input className={inputClass} placeholder="HDFC0001234" {...register('ifsc_code')} />
            </FormField>
          </div>
          ...
          {editData && (
            <div className="border-t border-zinc-800/50 pt-5 mt-2">
              <FormField label="Account Status">
                <select className={`${inputClass} appearance-none`} {...register('status')}>
          ...
          <div className="flex gap-3 pt-6 border-t border-zinc-800 mt-2">
            ...
            <button type="submit" disabled={saving} className="flex-1 bg-indigo-600 hover:bg-indigo-500 text-white font-medium rounded-xl px-4 py-3 transition-all duration-200 shadow-lg shadow-indigo-500/25 disabled:opacity-70 disabled:cursor-not-allowed">
              ...
              ) : editData ? 'Update Distributor' : 'Create Distributor'}
            </button>
          </div>
        </form>
      </Modal>
      ...
      <ConfirmDialog 
        ...
        message="Are you sure you want to delete this distributor? All linked dealers must be removed or reassigned first. This action cannot be undone." 
        danger 
      />
